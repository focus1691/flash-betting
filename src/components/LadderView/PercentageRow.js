...
const PercentageRow = ({
    ...
    Object.values(orders).forEach(rfs => {
      rfs.forEach(order => {
        ...
        switch (order.strategy) {
          case "Back":
            newBackList[order.selectionId] = newBackList[
              order.selectionId
            ].filter(item => item.rfs !== order.rfs);
            break;
          case "Lay":
            newLayList[order.selectionId] = newLayList[
              order.selectionId
            ].filter(item => item.rfs !== order.rfs);
            break;
          case "Stop Entry":
            newStopEntryList[order.selectionId] = newStopEntryList[
              order.selectionId
            ].filter(item => item.rfs !== order.rfs);
            break;
          case "Tick Offset":
          ...
          case "None":
            ...
            if (fillOrKillList[order.betId] !== undefined) {
              ordersToRemove = ordersToRemove.concat(
                newFillOrKill[order.betId]
              );
  ...
  const cancelAllOrdersOnSide = (
    ...
    if (currentOrders) {
      ...
      const cancelBets = await openSelectedRunnerOrders.reduce(
        async (previousPromise, nextOrder) => {
          ...
          return cancelOrderAction({
            ...
            unmatchedBets:
              previousCancelOrderUnmatchedBets &&
  ...
  return (
    <div className={"percentage-row"}>
      ...
      {/* Cancel Back/Lay */}
      <Tooltip
        title={`Cancel selection ${leftSide === "lay" ? "lay" : "back"} bets`}
        aria-label="Cancel selections"
      >
        <div
          className={"th"}
      ...
      <div
        className={"th"}
        style={{ backgroundColor: leftSide === "lay" ? "#FCC9D3" : "#BCE4FC" }}
      >
        {`${percent[leftSide]}%`}
      </div>
      {/* LTP */}
      <Tooltip title={`Swap Back/Lay Columns`} aria-label="Swap matched columns">
        <div
          className={"th"}
          style={ltpStyle}
          onClick={onChangeLadderSideLeft(
            ladderSideLeft === "LAY" ? "BACK" : "LAY"
          )}
        >
          {ltp[0]}
        </div>
      ...
      <div
        className={"th"}
        style={{ backgroundColor: leftSide === "lay" ? "#BCE4FC" : "#FCC9D3" }}
      >
        {`${percent[leftSide === "lay" ? "back" : "lay"]}%`}
      </div>
      {/* Cancel Lay/Back */}
      <Tooltip
        title={`Cancel selection ${leftSide === "lay" ? "back" : "lay"} bets`}
        aria-label={"Cancel selections"}
      >
        <div
          className={"th"}
