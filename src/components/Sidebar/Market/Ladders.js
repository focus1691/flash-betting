...
const Ladder = props => {
  ...
  const handleRunnerSelection = selectionId => e => {
    ...
    if (props.excludedLadders.indexOf(selectionId) === -1) {
      ...
      props.onChangeExcluded(
        props.excludedLadders.filter(item => item !== selectionId)
      );
    }
  };
  ...
  const renderRunners = () => {
    return props.sortedLadder.map(selectionId => {
      const { atb, atl, ltp, ltpStyle } = deconstructLadder(
        props.ladder[selectionId]
      );
      return (
...
const mapDispatchToProps = dispatch => {
  return {
    onChangeExcluded: excludedLadders =>
      dispatch(updateExcludedLadders(excludedLadders)),
