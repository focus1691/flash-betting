...
const GridDetailCell = props => {
	...
	const side =
		selectionMatchedBets.reduce((a, b) => a + calcBackProfit(b.size, b.price, b.side === "BACK" ? 0 : 1), 0) <= 0
			? "BACK"
			: "LAY";
		...
		if (isHedgingOnSelectionAvailable(props.market.marketId, props.runner.selectionId, props.bets)) {
			...
			const hedgeSize =
				selectionMatchedBets.length > 0
					? CalculateLadderHedge(props.ltp[0], selectionMatchedBets, "hedged").size
					: undefined;
	...
	return (
		<td className="grid-runner-details" onClick={props.onSelectRunner(props.runner)}>
