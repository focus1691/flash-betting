...
const GridDetailCell = props => {
  const selectionMatchedBets = Object.values(props.bets.matched).filter(
    bet => parseInt(bet.selectionId) === parseInt(props.runner.selectionId)
  );

  const side =
    selectionMatchedBets.reduce(
      (a, b) => a + calcBackProfit(b.size, b.price, b.side === "BACK" ? 0 : 1),
      0
    ) <= 0
      ? "BACK"
      : "LAY";
  ...
  const executeHedgeBet = () => e => {
    if (
      isHedgingOnSelectionAvailable(
        props.market.marketId,
        props.runner.selectionId,
        props.bets
      )
    ) {
      ...
      const hedgeSize =
        selectionMatchedBets.length > 0
          ? CalculateLadderHedge(props.ltp[0], selectionMatchedBets, "hedged")
              .size
          : undefined;
  ...
  return (
    ...
    >
      ...
      <div className={"grid-pl"}>
        <span
          style={{
            color: !isHedgingOnSelectionAvailable(
              props.market.marketId,
              props.runner.selectionId,
              props.bets
            )
        ...
        <span>
          {props.tv[0] ? Math.floor(props.tv[0]).toLocaleString() : ""}
        </span>
      </div>
    </td>
  );
