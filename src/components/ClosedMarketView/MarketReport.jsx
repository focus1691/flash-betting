...
const MarketReport = ({ matchedBets, runners }) => {
  ...
  const rows = runners.map((runner) => {
    const win = matchedBets !== undefined
      ? getPLForRunner(runners.marketId, runner.selectionId, { matched: matchedBets }).toFixed(2)
      : 0;
    const lose = matchedBets !== undefined
      ? getLossForRunner(runners.marketId, runner.selectionId, { matched: matchedBets }).toFixed(2)
      : 0; // placeholder
    return {
                  ...
                  {columns.map((column) => {
                    ...
                    return (
                      <TableCell key={column.id} align={column.align}>
                        ...
                        <span style={{ color, fontWeight: isProfitOrLoss ? 'bold' : 'normal' }}>
