...
function Navigation(options) {
  ...
  $('#nav-body-tabs').on('click', '.nav-tabs-tab', function () {
    ...
    const session = $(`.nav-views-view[data-session="${sessionID}"]`)[0];
    ...
    const sessionID = $(this).parent('.nav-tabs-tab').data('session');
    const session = $('.nav-tabs-tab, .nav-views-view').filter(`[data-session="${sessionID}"]`);
  ...
  //
  $('#nav-body-tabs').on('click', '#nav-tabs-add', () => {
    let params;
    if (typeof options.newTabParams === 'function') {
      ...
      params = ['http://localhost:3001/dashboard', {
  ...
  // go back
  //
  $('#nav-body-ctrls').on('click', '#nav-ctrls-back', () => {
    NAV.back();
  });
  //
  // go forward
  //
  $('#nav-body-ctrls').on('click', '#nav-ctrls-forward', () => {
    NAV.forward();
  });
  //
  // reload page
  ...
  this._addEvents = function (sessionID, options) {
    const currtab = $(`.nav-tabs-tab[data-session="${sessionID}"]`);
    const webview = $(`.nav-views-view[data-session="${sessionID}"]`);
    ...
    webview.on('enter-html-full-screen', () => {
      ...
      $('.nav-views-view.active').parents().not('script').siblings()
        .hide();
    });
    webview.on('leave-html-full-screen', () => {
      $('.nav-views-view.active').siblings().not('script').show();
      $('.nav-views-view.active').parents().siblings().not('script')
        .show();
    });
...
Navigation.prototype.newTab = function (url, options) {
  ...
  if (typeof options.newTabCallback === 'function') {
    const result = options.newTabCallback(url, options);
    if (!result) {
      return null;
    }
    if (result.url) {
  ...
  // build tab
  let tab = `<span class="nav-tabs-tab active" data-session="${this.SESSION_ID}">`;
  // favicon
  if (options.icon == 'clean') {
  ...
  const newWebview = this._addEvents(this.SESSION_ID++, options);
...
Navigation.prototype.closeTab = function (id) {
  ...
  if (id == null) {
    ...
    const sessionID = $(`#${id}`).data('session');
...
Navigation.prototype.nextTab = function () {
  const tabs = $('.nav-tabs-tab').toArray();
  const activeTabIndex = tabs.indexOf($('.nav-tabs-tab.active')[0]);
  let nexti = activeTabIndex + 1;
