...
class HttpRequest extends Stream {
  ...
  execute(cb = () => {
    ...
    const request = transport.request(httpOptions, (result) => {
  ...
  // http(s) end of chunk data
    ...
    const start = this.startTime[0] + (this.startTime[1] / NANOSECONDS_IN_SECOND);
    const end = this.endTime[0] + (this.endTime[1] / NANOSECONDS_IN_SECOND);

    // gzip compression efficiency
    const responseBodyLength = this.responseBody.length;
